...
public abstract class AbstractTween
{
	...
	protected Action<AbstractTween> _onComplete; // exectures whenever a Tween completes
	
	
	
	public void setOnStartHandler( Action<AbstractTween> onStart )
	{
		_onStart = onStart;
	}

	
	public void setOnCompleteHandler( Action<AbstractTween> onComplete )
	{
    ...
    {
        while( state != TweenState.Complete && state != TweenState.Destroyed )
            yield return null;
